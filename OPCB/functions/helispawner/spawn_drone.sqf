...
createDialog "jey_dronespawner";

waitUntil {
  !isNull (findDisplay 9909) && {!(isNil "OPCB_econ_initDone") && {OPCB_econ_initDone}}
};

_drones = [];

_ctrl = (findDisplay 9909) displayCtrl 1500;
_imageCtrl = (findDisplay 9909) displayCtrl 1618;

{
	if ((["DRONE", _x] call OPCB_econ_fnc_getVehicleTier) >= OPCB_econ_currentTier) then {
		_drones pushBack _x;
	};
} foreach OPCB_econ_vehicleTypes_DRONE;
...
_ctrl ctrlAddEventHandler ["LBSelChanged",{
	...
	_ctrl = (findDisplay 9909) displayCtrl 1001;	
	_tier = ["DRONE", _classname] call OPCB_econ_fnc_getVehicleTier;
