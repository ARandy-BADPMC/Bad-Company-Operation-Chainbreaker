...
createDialog "jey_boatspawner";

waitUntil {
  !isNull (findDisplay 74810) && { !(isNil "OPCB_econ_initDone") && {OPCB_econ_initDone}}
};

private _vehicles = [];
{
	if ((["SEA", _x] call OPCB_econ_fnc_getVehicleTier) >= OPCB_econ_currentTier) then {
		_vehicles pushBack _x;
	};
} foreach OPCB_econ_vehicleTypes_SEA;

_ctrl = (findDisplay 74810) displayCtrl 1500;
_imageCtrl = (findDisplay 74810) displayCtrl 1608;
...
_ctrl ctrlAddEventHandler ["LBSelChanged",{
	...
	_ctrl = (findDisplay 74810) displayCtrl 1001;
	_tier = ["SEA", _classname] call OPCB_econ_fnc_getVehicleTier;
