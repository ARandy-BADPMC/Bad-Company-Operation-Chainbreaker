	...
	for "_i" from 0 to count _houses -1 do {
		...
		if(_chance == 1 || _houses select (count _houses -1) == _item) then
		{
			...
			if( (_fugitivechance == 1 || _houses select (count _houses -1) == _item) && _isFugitive == 0) then{
				...
				[_unit,["<t color='#FF0000'>Do you know who stole the crate?</t>", "functions\code.sqf", [_theFugitive], 1.5, true, true, "", "alive _target", 6, false, ""]] remoteexeccall ["addaction",0,true];
 				[_unit,["<t color='#FF0000'>Do you know where is the crate?</t>", "functions\code2.sqf", [_crateHouse], 1.5, true, true, "", "alive _target", 6, false, ""]] remoteexeccall ["addaction",0,true];
			};
	...
	for "_i" from 0 to count _houses -1 do {
		...
		if(_chance == 1 || _houses select (count _houses -1) == _item) then
		{
			...
			//[_unit,_item,_crateHouse,_theFugitive] remoteExecCall ["JeyR_fnc_retrieve_addaction",0,true]; //[civ,[blablabla]] remotexeccall ["addaction",0,true]
			_unit setVariable ["isQuestioned", "no", true];
			_unit setVariable ["isQuestionedFugitive", "no", true];
