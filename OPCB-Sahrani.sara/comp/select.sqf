...
_tasks = ["Eliminate","Technology","Destroy","Annihilate and Destroy","Secure","Capture","Exterminate","Neutralize","Neutralize2","Attack","Clear out","IDAP"];
_tasks call BIS_fnc_arrayShuffle;
_taskobjective = selectRandom _tasks;
...
{ 
	case "Prison" :
	{
		...
		[_target1,10,1,2] execVM "functions\spawn_ins.sqf"; // this will also move into the functions library
	...
	//Was this task made by me? 
		...
		} forEach (allUnits select { (_x distance _guardpos ) < 200 } );*/ //this doesn't delete anything since no units were spawned yet
	...
	case "Neutralize" : //test it
	{
		...
		sleep 10;
		[_leader,10,1,2] execVM "functions\spawn_ins.sqf";
		sleep 5;
		waitUntil 
		{
			sleep 10;
	...
	case "Resupply" : //this will be remade
	{
		missionNamespace setVariable ["running_task",1];
	...
	case "Escort" : //this will also be remade. 
	{
		missionNamespace setVariable ["running_task",1];
		_taskcomp = selectRandom ["fob1","fob2","fob3","fob4","fob5"];
		[_current_tasknumber ,west,["An army officer is arriving soon to the airport. You have to escort him to a nearby FOB.","Escort"], _current_task,"ASSIGNED",10,true,true,"defend",true] call BIS_fnc_setTask;
		_officergroup = createGroup west;
	...
	case "Minefield" : //does this not work? Can somebody test it pls? the mines will be marker with a marker on a road
	{	
		missionNamespace setVariable ["running_task",1];
		_cities = missionNamespace getVariable["Cities",0];
		_city = selectRandom _cities;
